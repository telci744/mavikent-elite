...
const AdminScreen = ({ appData, goBackToRoles }) => {
  ...
  // ŞİFRE YÖNETİMİ İÇİN PIN'LER EKLENDİ
  ...
  const completeQuest = (qId) => {
    ...
    if(window.confirm(`${q.participants.length} öğrenciye ödülleri dağıtılacak. Onaylıyor musun?`)) {
      q.participants.forEach(p => {
        ...
        if(q.type === 'M') { db.ref(`mavikent_premium/wallet/${p}`).transaction(c => (c||0) + parseInt(q.amt)); db.ref(`mavikent_premium/xp/${p}`).transaction(c => (c||0) + (parseInt(q.amt) * 10)); } 
        else { db.ref(`mavikent_premium/season_score/${p}`).transaction(c => (c||0) + parseInt(q.amt)); }
      });
  ...
  const editProduct = (key, prod) => {
      setNewProduct({ name: prod.n, price: prod.p, icon: prod.i, type: prod.type || 'normal' }); setEditProductKey(key); window.scrollTo(0,0);
  };
  ...
  const distributeExamRewards = (type) => {
    ...
    roster.forEach(n => {
       ...
       if (score !== undefined && d.target && parseFloat(score) >= parseFloat(d.target)) { db.ref(`mavikent_premium/wallet/${n}`).transaction(c => (c||0) + reward); db.ref(`mavikent_premium/season_score/${n}`).transaction(c => (c||0) + reward); count++; }
    });
  ...
  const handleGlobalXpReset = () => {
      ...
      if (confirmText === 'SIFIRLA') { const updates = {}; roster.forEach(n => { updates[`xp/${n}`] = 0; }); db.ref('mavikent_premium').update(updates); }
  };
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
            ...
            {dashboardView === 'egitim_yazili' && (
              <>
                ...
                <div style={{ gridColumn: '1 / -1', marginTop: '10px' }}><button onClick={() => { if(window.confirm('YAZILI notları sıfırlanacak. Emin misiniz?')) { const updates = {}; roster.forEach(n => updates[`exams/${n}/yazili`] = null); db.ref('mavikent_premium').update(updates); alert('Yazılı notları sıfırlandı!'); } }} className="premium-btn" style={{ width: '100%', background: '#ef4444', color: 'white', padding: '18px', fontSize: '15px' }}>🔄 HAFTALIK YAZILI PERFORMANSLARINI SIFIRLA</button></div>
              </>
            )}
            ...
            {dashboardView === 'yonetim' && [ { id: 'admin_students', icon: '👥', label: 'ÖĞRENCİ' }, { id: 'admin_quests', icon: '🎯', label: 'GÖREVLER' }, { id: 'admin_market', icon: '🛒', label: 'MARKET' }, { id: 'admin_teslimat', icon: '📦', label: 'TESLİMAT' }, { id: 'admin_lig', icon: '🏆', label: 'ELİT LİG' }, { id: 'admin_settings', icon: '⚙️', label: 'AYARLAR' } ].map(mod => (
          ...
          <div style={{ background: 'white', padding: '24px', borderRadius: '24px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)' }}>
             ...
             <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '12px', marginBottom: '24px', background:'#f8fafc', padding:'20px', borderRadius:'24px' }}>
               ...
               <select value={newProduct.type} onChange={e => setNewProduct({...newProduct, type: e.target.value})} className="elite-input" style={{ width: '100%' }}>
                 <option value="normal">🍔 Normal</option><option value="avatar">👤 Avatar</option><option value="multiplier">⚡ 2X Kartı</option><option value="title">🎖️ Ünvan</option><option value="frame">🖼️ Çerçeve</option>
               </select>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
            <div style={{ background: '#fffbeb', border: '2px solid #fde047', padding: '30px', borderRadius: '24px' }}>
               ...
               <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                 {roster.filter(isElite).map(n => <div key={n} onClick={() => db.ref(`mavikent_premium/student_tiers/${n}`).set('standard')} className="premium-hover" style={{ background: '#fef3c7', padding: '12px 16px', borderRadius: '14px', fontSize: '14px', fontWeight: 800, color:'#92400e' }}>{n} 👑</div>)}
               </div>
            </div>
            <div style={{ background: '#ffffff', padding: '30px', borderRadius: '24px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)' }}>
               ...
               <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                 {roster.filter(n => !isElite(n)).map(n => <div key={n} onClick={() => db.ref(`mavikent_premium/student_tiers/${n}`).set('elite')} className="premium-hover" style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: '12px 16px', borderRadius: '14px', fontSize: '14px', fontWeight: 800, color:'#334155' }}>{n} ⬆️</div>)}
               </div>
            </div>
          </div>
        )}

        {/* --- YÖNETİCİ AYARLAR PANELİ (ŞİFRE DEĞİŞTİRME EKLENDİ) --- */}
        {currentModule === 'admin_settings' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
             ...
             <div style={{ background: 'white', padding: '30px', borderRadius: '24px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)' }}>
               ...
               <div style={{ fontSize: '13px', color: '#64748b', marginBottom: '16px', fontWeight: 600 }}>Yönetici ve Personel giriş PIN kodlarını belirleyin.</div>
